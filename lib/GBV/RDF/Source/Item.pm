...
sub retrieve_rdf {
    ...
    return
      unless $uri =~
      qr{^http://uri\.gbv\.de/document/(opac-[a-zA-Z0-9:/-]+):epn:(\d+)$};
    my ( $isil, $epn ) = ( $1, $2 );
...
#    push @triples, [ iri($uri), iri('http://example.org/described-by-record-in-database'), iri($opacuri) ];
    ...
    if ($picabase) {
        my $url =
          $picabase->uri . "CMD?ACT=SRCHA&IKT=1016&TRM=epn+$epn&XML=1.0";
        my $xml = get($url);
        if ( $xml =~ /PPN="([0-9x]+)"/mi ) {
            my $ppn = $1;
    ...
    push @triples,
      GBV::RDF::Source::Document->pica_data( $uri, $pica, $picabase );
...
sub picaitem_to_triples {
    ...
    if ( $data->{picabase} ) {
        my $url =
            $data->{picabase}->uri
          . "CMD?ACT=SRCHA&IKT=1016&TRM=epn+"
          . $item->epn;
